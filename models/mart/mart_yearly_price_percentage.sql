...
YearlyGrowth AS (
    SELECT
        ...
        avg_sales_price_m_2,
        safe_divide((avg_sales_amount - LAG(avg_sales_amount) OVER (PARTITION BY municipality_code, premise_type ORDER BY sales_year)), LAG(avg_sales_amount) OVER (PARTITION BY municipality_code, premise_type ORDER BY sales_year)) AS pct_growth_sales_amount,
